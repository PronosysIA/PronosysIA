...
function SectionLabel({ children }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "16px" }}>
      <div style={{ width: "20px", height: "1px", background: "#C6A15B", flexShrink: 0 }} />
      <span style={{ fontSize: "10px", fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "#666" }}>
        {children}
      </span>
    </div>
  );
  ...
  return (
    <div
      ...
      style={{
        ...
        background: "#0D0D0D",
        border: hovered && available ? "1px solid rgba(198,161,91,0.25)" : "1px solid #1A1A1A",
        borderRadius: "14px",
        padding: "28px 24px",
        ...
        boxShadow: hovered && available ? "0 12px 40px rgba(0,0,0,0.4)" : "none",
        position: "relative",
        overflow: "hidden",
      }}
  ...
  return (
    <div
      ...
      style={{
        animationDelay: delay,
        background: "#0D0D0D",
        border: hovered && unlocked ? "1px solid rgba(198,161,91,0.2)" : "1px solid #1A1A1A",
        borderRadius: "12px",
        padding: "22px 20px",
...
export default function Dashboard() {
  ...
  return (
    <DashboardLayout>
      {message && (
        <div className="animate-fadeIn" style={{ background: "rgba(74,222,128,0.06)", border: "1px solid rgba(74,222,128,0.15)", borderRadius: "10px", padding: "14px 18px", marginBottom: "32px", fontSize: "13px", color: "#4ADE80" }}>
          {message}
        </div>
      )}
      ...
      <div style={{ marginBottom: "40px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(1, 1fr)", gap: "10px" }}>
          <AnalysisCard title="Pubs Classiques" desc="Meta Ads, Google Ads, YouTube Ads et autres regies publicitaires." href="/dashboard/pubs" available={canPubs} lockMsg="Plan Power ou Combo requis" delay="0ms" />
          <AnalysisCard title="Videos Reseaux" desc="TikTok, Instagram Reels, YouTube Shorts, Snapchat et autres." href="/dashboard/reseaux" available={canReseaux} lockMsg="Plan Creator ou Combo requis" delay="80ms" />
        </div>
      </div>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(1, 1fr)", gap: "10px" }}>
          <ToolCard label="Generateur" desc="Script video ameliore seconde par seconde, hooks viraux, musiques." href="/dashboard/generator" icon="✨" unlocked={isPremium} delay="0ms" />
          <ToolCard label="Boosteur" desc="Horaires optimaux, hashtags, captions et strategie cross-posting." href="/dashboard/booster" icon="🚀" unlocked={isPremium} delay="60ms" />
          <ToolCard label="Chatbot Video" desc="Discutez avec l'IA pour creer et generer la video parfaite." href="/dashboard/chatbot" icon="🤖" unlocked={isCombo} comboOnly delay="120ms" />
        </div>
      </div>
    </DashboardLayout>
  );
